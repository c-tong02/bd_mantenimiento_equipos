...
SELECT
	e.id as 'id_dim_equipos',
	t.id as 'id_dim_tecnicos',
	fecha_mantenimiento as 'id_dim_fechas',
	DATEDIFF(hour, fecha_inicio, fecha_fin) AS 'duracion_total_mantenimiento',
	SUM(ur.cantidad_usada) AS 'cantidad_repuestos_usados',
	SUM(r.costo_unitario * ur.cantidad_usada) AS 'costo_repuestos',
	t.costo_hora*DATEDIFF(hour, fecha_inicio, fecha_fin) AS 'costo_operativo',
	t.costo_hora*DATEDIFF(hour, fecha_inicio, fecha_fin) + SUM(r.costo_unitario * ur.cantidad_usada) AS 'costo_total_mantenimiento'
FROM mantenimientos m
	INNER JOIN equipos e ON e.id = m.id_equipo
	INNER JOIN uso_de_repuestos ur ON m.id = ur.id_mantenimiento
	INNER JOIN tecnicos t ON t.id = m.id_tecnico
...
GROUP BY
	...
	t.id,
	fecha_mantenimiento,
	DATEDIFF(hour, fecha_inicio, fecha_fin),
	t.costo_hora*DATEDIFF(hour, fecha_inicio, fecha_fin);